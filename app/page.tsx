 ...
 *   rewritten CV, real PDF download, feature grid, pricing teaser, trust line.
 *
...
const FEATURES = [
  ...
  },
  {
    icon: "🌍",
    title: "Japa Mode",
    desc: "UK, Canada & US format CVs — coming soon.",
  },
  {
    icon: "⚡",
    title: "60 Seconds",
    desc: "Paste, fix, download. No account needed.",
  },
];
...
const PRICING = [
  {
    plan: "Free",
    price: "₦0",
    perks: ["1 CV/month", "Watermarked PDF"],
  },
  {
    plan: "Starter",
    price: "₦1,500",
    perks: ["1 clean CV", "No watermark", "PDF download"],
    featured: true,
  },
  {
    plan: "Pro",
...
export default function Home() {
  ...
  // ---- Real PDF download via @react-pdf/renderer ----
  async function downloadPdf() {
    if (!result) return;
    setPdfLoading(true);
    try {
      const blob = await pdf(
        <CvDocument cvText={result.rewrittenCv} />
      ).toBlob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "CVPro-Resume.pdf";
  ...
  // ---- Copy rewritten CV to clipboard ----
    ...
    <div className="mx-auto w-full max-w-2xl px-4 pb-16">
      ...
      <section className="mt-14">
        ...
        <p className="mt-4 text-center text-xs text-gray-500">
          Payments via Paystack — cards, bank transfer &amp; USSD. Coming in
          Phase 2.
        </p>
      </section>
